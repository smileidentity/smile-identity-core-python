...
def generate_zip_file(
    ...
    with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
        ...
        for image in image_params:
            ...
            if isinstance(image_file_path, str) and image_file_path.lower().endswith(IMAGE_FILE_EXTENSIONS):
...
def validate_images(images_params):
    ...
    for image in images_params:
        if isinstance(image["image"], str) and image["image"].lower().endswith(IMAGE_FILE_EXTENSIONS):
